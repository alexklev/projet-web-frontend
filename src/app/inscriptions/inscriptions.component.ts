...
export class InscriptionsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.displayedColumns = ['nom', 'niveau', 'date_debut', 'date_fin', 'action'];
    this.dtOption = {
      destroy: true,
      "paging":   true,
      "ordering": true,
  ...
  getformation(){
    ...
    data.append('idutilisateur',localStorage.getItem('idetudiant')!);
    this.http.post<any>(environment.base_url+'searchInscriptionByFormation',data).subscribe((data1:any) => {
  ...
  removeinscription(elt:any){
    if(confirm("Voulez-vous vraiment supprimer cette demande? ")) { const options = {
      headers: new HttpHeaders({
        'Content-Type': 'application/json',
      }),
    };
      this.http.delete<any>(environment.base_url+'inscription/'+elt.idinscription,options).subscribe(data => {
        this.getformation();
        // location.reload();
      },
      error =>
  ...
  exportTable(type:any){
    if (type == 'xlsx') {
      ...
      /* save to file */
      XLSX.writeFile(wb, 'Mes_inscriptions.xlsx');
    }
    if (type == 'csv') {
      ...
      FileSaver.saveAs(blob,'Mes_inscriptions');
    }
    if (type == 'pdf') {
