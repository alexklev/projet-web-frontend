...
export class CoursComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.role == 'admin') {
      this.displayedColumns = ['nom', 'actif','nbformation', 'action'];
    }
    else{
      this.displayedColumns = ['nom','nbcours', 'action'];
    }
    this.dtOption = {
      destroy: true,
      "paging":   true,
  ...
  getformation(){
    ...
    this.http.post<any>(environment.base_url+'searchCour',data).subscribe((data1:any) => {
  ...
  opendelete(elt:any){
    if(confirm("Voulez-vous vraiment supprimer ce cours? ")) { const options = {
      headers: new HttpHeaders({
        'Content-Type': 'application/json',
      }),
    };
      this.http.delete<any>(environment.base_url+'cours/'+elt.id,options).subscribe(data => {
        this.getformation();
        // location.reload();
      },
      error =>
  ...
  openupdate(elt:any){
    const dialogRef = this.dialog.open(AjoutercComponent, {
      width: '800px',
      data: {'client':elt}
    });
  ...
  opencours(elt:any){
    const dialogRef = this.dialog.open(FormationComponent, {
      width: '800px',
      data: {'formation':elt.formation,titre:elt.nom}
    });
